  ...
  return <main className={styles.page}>
    ...
    <div className={styles.grid}>
      ...
      <section className={styles.detail}>
        {task ? <>
          <div className={styles.detailHead}><div><span className={styles.badge} data-level={task.risk_level}>{task.risk_level}</span><h2>{task.user_utterance}</h2><p>Task ID · {task.id}</p></div><strong data-status={task.status}>{task.status}</strong></div>
