...
type ChannelEvent = { id: string; task_id?: string; direction: string; event_type: string; outcome: string; created_at: string };
    ...
    <div className={styles.grid}>
      ...
      <section className={styles.detail}>
        {task ? <>
          ...
          {channelEvents.length > 0 && <><div className={styles.subhead}><GitBranch size={17} /><b>渠道事件</b><span>{channelEvents.length} 条</span></div><div className={styles.dag}>{channelEvents.map((event) => <div className={styles.node} key={event.id}><div className={styles.nodeIndex}>企</div><div><b>{event.event_type}</b><code>{event.direction} · {event.outcome}</code></div><span>{new Date(event.created_at).toLocaleTimeString("zh-CN")}</span><Check size={16} /></div>)}</div></>}
