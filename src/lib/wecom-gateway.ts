...
export async function handleWecomMessage(input: WecomMessageInput) {
  ...
  if (!identity) {
    ...
    const bindUrl = `${configuredPublicUrl()}/connect/wecom#token=${encodeURIComponent(binding.token)}`;
  ...
  await query(
    `UPDATE channel_events SET channel_identity_id=$2,customer_id=$3
     WHERE channel_type='WECOM' AND direction='INBOUND' AND external_event_id=$1`,
    [input.eventId, identity.id, identity.customer_id],
  );
...
export async function handleWecomAction(input: WecomActionInput) {
  const identity = await resolveChannelIdentity({
    channelType: "WECOM",
    tenantExternalId: input.tenantExternalId,
    subjectExternalId: input.subjectExternalId,
  ...
  if (!run || run.operation_id !== input.operationId || run.channel_type !== "WECOM" || run.channel_identity_id !== identity.id) {
